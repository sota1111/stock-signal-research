  ...
  return (
    <div className="space-y-8">
      ...
      <nav aria-label="breadcrumb" className="flex items-center gap-1.5 text-sm text-muted-foreground flex-wrap">
        ...
        <span aria-hidden className="text-gray-300">/</span>
        <span className="text-foreground font-medium truncate max-w-[16rem]">{theme.name}</span>
      </nav>
      ...
      <section className="bg-surface rounded-lg shadow p-6">
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-muted-foreground mt-1">{theme.category}</p>
            {theme.description && <p className="text-foreground mt-3">{theme.description}</p>}
          </div>
          <div className="text-right space-y-2">
            <div>
              ...
              <span className="block text-xs text-muted-foreground mt-0.5">{t('detail.summary.precursor')}</span>
            </div>
            ...
            {alignment && alignment.evidence_count > 0 && (
              <div>
                ...
                <span className="block text-xs text-muted-foreground mt-0.5">{t('detail.summary.alignment')}</span>
              </div>
            )}
      ...
      <div className="flex flex-wrap gap-2 border-b">
        {detailTabs.map(tb => (
          <button
            ...
            className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${detailTab === tb.id ? 'border-blue-600 text-blue-600' : 'border-transparent text-muted-foreground hover:text-foreground'}`}
          >
      ...
      <section className="bg-surface rounded-lg shadow p-4">
        <p className="text-sm font-semibold text-muted-foreground mb-3">{t('detail.flow.title')}</p>
        <div className="flex flex-wrap items-center gap-2 text-sm">
          <Link to={`/papers?theme=${encodeURIComponent(theme.name)}`} className="rounded-md border border-gray-300 bg-surface px-3 py-1.5 text-foreground hover:bg-surface-muted">{t('nav.papers')}</Link>
          <span className="text-gray-300" aria-hidden>→</span>
          <Link to={`/patents?theme_id=${encodeURIComponent(theme.id)}`} className="rounded-md border border-gray-300 bg-surface px-3 py-1.5 text-foreground hover:bg-surface-muted">{t('nav.patents')}</Link>
          <span className="text-gray-300" aria-hidden>→</span>
          <Link to={`/investors?theme=${encodeURIComponent(theme.name)}`} className="rounded-md border border-gray-300 bg-surface px-3 py-1.5 text-foreground hover:bg-surface-muted">{t('nav.investors')}</Link>
          <span className="text-gray-300" aria-hidden>→</span>
      ...
      {detailTab === 'overview' && (
      <section className="bg-surface rounded-lg shadow p-6">
        <h2 className="text-lg font-semibold text-foreground mb-4">{t('detail.monthlyPapers')}</h2>
      ...
      {detailTab === 'related' && (
        <section className="bg-surface rounded-lg shadow p-6 space-y-6">
          {/* 関連テーマ間ナビ（SOT-995 /themes-4） */}
          <div>
            <h2 className="text-lg font-semibold text-foreground mb-3">{t('detail.relatedThemes')}</h2>
          ...
          {/* サプライチェーン関連 */}
          <div>
            <h2 className="text-lg font-semibold text-foreground mb-3">{t('detail.supplyChainRelated')}</h2>
            {relatedSC.length > 0 ? (
              <div className="space-y-2">
                {relatedSC.map(sc => (
                  <div key={sc.id} className="flex items-center gap-3 text-sm">
                    ...
                    {sc.description && <span className="text-muted-foreground text-xs">({sc.description})</span>}
                  </div>
                ))}
              </div>
            ) : (
      ...
      {detailTab === 'overview' && alignment && alignment.evidence_count > 0 && (
        <section className="bg-surface rounded-lg shadow p-6">
          <h2 className="text-lg font-semibold text-foreground mb-4">{t('detail.alignment.title')}</h2>
          <div className="flex items-center gap-6 flex-wrap">
            <div className="text-center">
              <div className={`text-3xl font-bold ${alignment.score >= 60 ? 'text-blue-600' : alignment.score >= 40 ? 'text-yellow-600' : 'text-muted-foreground'}`}>
                {alignment.score.toFixed(0)}
              </div>
              <div className="text-xs text-muted-foreground mt-1">{t('detail.alignment.totalScore')}</div>
            </div>
            <div className="flex gap-4 text-sm flex-wrap">
              <div className="text-center">
                <div className="font-semibold text-foreground">{alignment.news_score.toFixed(0)}</div>
                <div className="text-xs text-muted-foreground">{t('detail.alignment.news')}</div>
              </div>
              <div className="text-center">
                <div className="font-semibold text-foreground">{alignment.announcement_score.toFixed(0)}</div>
                <div className="text-xs text-muted-foreground">{t('detail.alignment.announcement')}</div>
              </div>
              <div className="text-center">
                <div className="font-semibold text-foreground">{alignment.earnings_score.toFixed(0)}</div>
                <div className="text-xs text-muted-foreground">{t('detail.alignment.earnings')}</div>
              </div>
            </div>
            <div className="text-sm">
              <span className={`px-2 py-1 rounded text-xs font-medium ${alignment.confidence >= 0.8 ? 'bg-green-100 text-green-700' : alignment.confidence >= 0.5 ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-muted-foreground'}`}>
          ...
          <p className="text-xs text-muted-foreground mt-3">{t('detail.alignment.disclaimer')}</p>
        </section>
      )}
      ...
      {detailTab === 'external' && externalInfos && ([
        ...
        <section key={group.key} className="bg-surface rounded-lg shadow p-6">
          <h2 className="text-lg font-semibold text-foreground mb-4">{t(group.label, { n: group.items.length })}</h2>
              ...
              <div key={item.id} className="border-b pb-3">
                ...
                <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground mt-1">
                  ...
                  {item.relevance_score > 0 && (
                    <span className="text-muted-foreground">{t('detail.external.relevance', { n: Math.round(item.relevance_score) })}</span>
                  )}
                </div>
      ...
      <section className="bg-surface rounded-lg shadow p-6">
        <div className="flex flex-wrap items-center justify-between gap-2 mb-4">
          <h2 className="text-lg font-semibold text-foreground">{t('detail.relatedPapers', { n: papers?.length ?? 0 })}</h2>
        ...
        {papers && papers.length > 0 ? (
          <div className="space-y-3">
            {papers.map(p => (
              <div key={p.id} className="border-b pb-3">
                ...
                <p className="text-xs text-muted-foreground mt-0.5">{p.published_at} · {p.source}</p>
          ...
          <p className="text-muted-foreground text-sm">{t('detail.noRelatedPapers')}</p>
        )}
      </section>
      )}
    </div>
