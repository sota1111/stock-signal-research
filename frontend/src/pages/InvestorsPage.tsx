  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">{t('investors.subtitle')}</p>
      </div>

      <section>
        <div className="flex flex-wrap items-center justify-between gap-2 mb-1">
          <h2 className="text-lg font-semibold text-foreground">{t('investors.institutional')}</h2>
          {allInvestors.length > 0 && (
            <button
              onClick={() => setLatestOnly(v => !v)}
              className="text-xs px-3 py-1 rounded border border-gray-300 text-muted-foreground hover:bg-surface-muted"
            >
              {latestOnly ? t('investors.showAll') : t('investors.latestOnly')}
            </button>
          )}
        </div>
        <p className="text-sm text-muted-foreground mb-3">{t('investors.institutional.subtitle')}</p>
              ...
              <tbody>
                {investorRows.map(inv => (
                  <tr key={inv.id} className="border-t hover:bg-surface-muted">
                    ...
                    <td className="px-4 py-2 text-muted-foreground" data-label={t('investors.col.ticker')}>{inv.ticker ?? '—'}</td>
                    <td className="px-4 py-2 text-muted-foreground font-mono text-xs" data-label={t('investors.col.cusip')}>{inv.cusip ?? '—'}</td>
                    <td className="px-4 py-2 text-right" data-label={t('investors.col.shares')}>{inv.shares != null ? inv.shares.toLocaleString() : '—'}</td>
                    <td className="px-4 py-2 text-right text-muted-foreground" data-label={t('investors.col.value')}>{inv.value_usd != null ? `$${Math.round(inv.value_usd).toLocaleString()}` : '—'}</td>
                    ...
                    <td
                      className={`px-4 py-2 text-right ${inv.quarter_delta == null || inv.quarter_delta === 0 ? 'text-muted-foreground' : inv.quarter_delta > 0 ? 'text-green-600' : 'text-red-600'}`}
      ...
      {allInvestors.length > 0 && (
        <section className="space-y-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div>
              <h2 className="text-lg font-semibold text-foreground">{t('investors.trend.title')}</h2>
              <p className="text-sm text-muted-foreground">{t('investors.trend.subtitle')}</p>
            </div>
            <div className="flex items-center gap-2">
              <label htmlFor="trend-company" className="shrink-0 text-sm text-muted-foreground">{t('investors.trend.selectCompany')}</label>
              <select
                id="trend-company"
      ...
      {relations.length > 0 && (
        <section className="space-y-3">
          <div>
            <h2 className="text-lg font-semibold text-foreground">{t('investors.relation.title')}</h2>
            <p className="text-sm text-muted-foreground">{t('investors.relation.subtitle')}</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {relations.map(r => (
              <div key={r.investor} className="bg-surface rounded-lg shadow p-4">
                <p className="font-semibold text-foreground mb-2">{r.investor}</p>
      ...
      <section>
        <h2 className="text-lg font-semibold text-foreground mb-3">{t('investors.top5')}</h2>
        {data.notable_companies.length === 0 ? (
          <p className="text-sm text-muted-foreground">{t('investors.noCompanies')}</p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {data.notable_companies.map(company => (
              <div key={company.id} className="bg-surface rounded-lg shadow p-4">
                <div className="flex justify-between items-center">
                  <div>
                    <p className="font-semibold">{company.name}</p>
                    {company.ticker && <p className="text-xs text-muted-foreground">{company.ticker}</p>}
                  </div>
                  <div className="text-right">
                    <ScoreBadge score={company.benefit_score} />
                    <p className="text-xs text-muted-foreground mt-1">
                      <span className={company.benefit_type === 'direct' ? 'text-blue-600' : 'text-muted-foreground'}>
      ...
      <section>
        <h2 className="text-lg font-semibold text-foreground mb-3">{t('investors.supplyChain')}</h2>
        <div className="bg-surface rounded-lg shadow p-4">
          {data.supply_chain_highlights.length === 0 ? (
            <p className="text-sm text-muted-foreground">{t('investors.noChain')}</p>
          ) : (
