...
MAX_PER_THEME = 25       # 1テーマあたり収集する実論文数
...
def main() -> int:
    ...
    print(f"[S2] enriching citations for {len(all_papers)} papers...", file=sys.stderr)
    enrich_citations(all_papers)

    with_cite = sum(1 for p in all_papers if p["citation_count"] > 0)
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(all_papers, f, ensure_ascii=False, indent=2)
    print(f"Wrote {len(all_papers)} real papers ({with_cite} with citation>0) to {out_path}")
    return 0
