...
def aggregate_paper_monthly_counts(
    ...
    さらに、各テーマ系列の **末尾に連続する空月(count==0)はトリム** し、系列が「そのテーマで
    論文が存在する最後の月」で終端するようにする。to_year は現在年で固定されるため、放置すると
    各系列が未来月(現在月以降のデータの無い月)で埋まり、消費側が `series[-1]` を直近月として
    使うと MoM/連続増/直近論文数がすべて0に潰れてしまう。先頭・中間の0月は連続性のため残し、
    末尾の0月のみ落とす。
    """
    ...
    for theme in sorted(themes_with_data):
        ...
        # 末尾の空月(未来月・直近のデータ無し月)をトリムし、実データのある最終月で終端する。
        # 先頭・中間の0月は連続性のため残す。themes_with_data は最低1件のデータを持つので
        # トリム後に空にはならない。
        while theme_rows and theme_rows[-1]["count"] == 0:
            theme_rows.pop()
        out.extend(theme_rows)
    return out
